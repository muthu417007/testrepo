({
    ...
    checkFeedback: function (component, event, helper, value) {
        var givenselectedPageTemplete = component.get("v.pageRatingCustomization.selectedPageTemplete")
        if (givenselectedPageTemplete == 'Emoticons' || givenselectedPageTemplete == 'Stars') {
            component.set("v.feedback_type", 1);
            var clicked = event.target;
            var newIndex = clicked.getAttribute("data-new-index");
            if (document.getElementsByClassName('su__ratingicon_star').length != 0) {
                for (var a = 0; a < document.getElementsByClassName('su__ratingicon_star').length; a++) {
                    document.getElementsByClassName('su__ratingicon_star')[a].classList.remove('su__emoji-active');
                    document.getElementsByClassName('su__ratingicon_star')[a].classList.remove('su__star-yellow');
                }
                for (var i = 0; i < newIndex; i++) {
                    document.getElementsByClassName('su__ratingicon_star')[i].classList.add('su__emoji-active');
                    document.getElementsByClassName('su__ratingicon_star')[i].classList.add('su__star-yellow');
                }
            }
            component.set("v.pageRateFeed", newIndex)
            // component.set("v.feedbackRatingV", rowIndex);
        } else {
            component.set("v.feedback_type", 0);
            // var checkIndex=clicked.getAttribute("data-value");
            // component.set("v.pageRateFeed",checkIndex)
        }
        ...
        var RegexSearch = component.get("v.searchPageRegex");
        var showSearchFeedback = component.get("v.pageRatingCustomization");
        if (value === "true" && (RegexSearch && showSearchFeedback.searchToggle)) {
            component.set("v.feedbackActiveYes", false);
            component.set("v.feedbackActiveNo", false);
            component.set('v.displayModal', true);
            component.set("v.pageRatingResponseRecorded", false);
            var comEmail = component.find("feedbackEmail") ? component.find("feedbackEmail").get("v.value") || component.get("v.currentCommunityEmail") : '';;
            if (comEmail.length != 0 && getFeedbackResDataStore.followUpToggle) {
                var currentEmailName = component.get("v.currentCommunityEmail");
                component.set("v.setCurrentEmail", currentEmailName);
                if (component.get("v.setCurrentEmail") !== '') {
        ...
        else if (value === "true" && (showSearchFeedback.pageFeedbackToggle && RegexSearch)) {
            component.set("v.feedbackActiveYes", false);
            component.set("v.feedbackActiveNo", false);
            component.set('v.TextdisplayModal', true);
            component.set("v.pageRatingResponseRecorded", false);
            getFeedbackResDataStore.selectedSearchAcknowledgement = showSearchFeedback.selectedAck;
            component.set("v.getFeedbackResData", getFeedbackResDataStore);
            var comEmail = component.find("feedbackEmail") ? component.find("feedbackEmail").get("v.value") || component.get("v.currentCommunityEmail") : '';;
            if (comEmail.length != 0 && getFeedbackResDataStore.followUpToggle) {
                var currentEmailName = component.get("v.currentCommunityEmail");
                component.set("v.setCurrentEmail", currentEmailName);
        ...
        else if (value === "true" && !showSearchFeedback.pageFeedbackToggle || (!RegexSearch && !showSearchFeedback.searchToggle)) {
            getFeedbackResDataStore.selectedSearchAcknowledgement = showSearchFeedback.selectedAck;
            component.set("v.getFeedbackResData", getFeedbackResDataStore);
            component.set("v.pageRatingResponseRecorded", true);
            helper.submitTextFeedback(component, event, helper);
        } else if (value === "false") {
    ...
    feedbackTxtChange: function (component, event, helper) {
        helper.feedbackTxtChange(component, event);
        // var feedEmailValCheck = component.get("v.feedbackTxtInVal");
        // if(feedEmailValCheck && feedEmailValCheck.trim() != ''){
        //     component.set('v.isButtonActive',false);
        // } else {
        //     component.set('v.isButtonActive',true);
        // }
    ...
    onChangeEmail: function (component, event, helper) {
        helper.onChangeEmail(component, event);
    ...
    enableField: function (component, event, helper) {
        component.set("v.emailTrackYes", true);
    },
    ...
    closeBtn: function (component, event, helper) { 
        if (document.getElementsByClassName('su__ratingicon_star').length != 0) {
            for (let i = 0; i < document.getElementsByClassName('su__ratingicon_star').length; i++) {
                document.getElementsByClassName('su__ratingicon_star')[i].classList.remove('su__emoji-active');
                    document.getElementsByClassName('su__ratingicon_star')[i].classList.remove('su__star-yellow');    
            }
        }
    ...
    starhoverButton: function (component, event, helper) {
        let hover = event.target;
        let newrowIndexdata = hover.getAttribute("data-new-index");
        if (document.getElementsByClassName('su__ratingicon_star').length != 0) {
            for (let i = 0; i < newrowIndexdata; i++) {
                document.getElementsByClassName('su__ratingicon_star')[i].classList.add('su__emoji-hover');
                document.getElementsByClassName('su__ratingicon_star')[i].classList.add('su__star-hover');
            }
        }
    },
    ...
    starremoveHoverButton: function (component, event, helper) {
        var hovered = event.target;
        var newrowIndexs = hovered.getAttribute("data-new-index");
        if (document.getElementsByClassName('su__ratingicon_star').length != 0) {
            for (let i = 0; i < newrowIndexs; i++) {
                document.getElementsByClassName('su__ratingicon_star')[i].classList.remove('su__emoji-hover');
                document.getElementsByClassName('su__ratingicon_star')[i].classList.remove('su__star-hover');
            }
        }
    },
