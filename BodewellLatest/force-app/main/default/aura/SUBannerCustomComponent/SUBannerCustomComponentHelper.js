({
    ...
    getURLParameter: function (param) {
        ...
        if(param == "searchString"){
          ...
          result = decodeURIComponent((new RegExp('[#|?|&]' + param + '=' + '([^&;]+?)(&|#|;|$)').exec(decodeURIComponent(s)) || [, ""])[1].replace(/\+/g, '%20'))
          
        return result;
    },
    redirectFunc: function (component, SearchQuery, helper) {
        var urlEvent = $A.get("e.force:navigateToURL");
        var url = component.get("v.currentCommunityURL") + "?searchString=" + encodeURIComponent(SearchQuery) + window.location.hash;
    ...
    enterKeySearchFunc: function (component, event, helper) {
        ...
        if (event.getParams().domEvent.keyCode == 13) {
            ...
            if(event.getParams().domEvent.keyCode == 40 || event.getParams().domEvent.keyCode == 38){
                ...
                if(event.getParams().domEvent.keyCode == 40 && index < length){
                    index++;
                    component.set('v.index',index);
                    document.getElementsByClassName("su-autoSuggest-element")[index].classList.add('selected');
                    var newSearchQuery = document.getElementsByClassName("su-autoSuggest-element")[index].getElementsByTagName("a")[0].id 
                    component.set("v.searchString",newSearchQuery);
                }else if(event.getParams().domEvent.keyCode == 38 && index >-1){
                    index--;
                    component.set('v.index',index);
                    document.getElementsByClassName("su-autoSuggest-element")[index].classList.add('selected');
                    var newSearchQuery = document.getElementsByClassName("su-autoSuggest-element")[index].getElementsByTagName("a")[0].id 
    ...
    autosearchFunc: function (component, searchType, runLoader, helper) {
        ...
        action.setParams({
            "searchParams": {
                ...
                "selectedType": helper.getURLParameter('selectedType') || aggrValue,
                "referrer": "",
