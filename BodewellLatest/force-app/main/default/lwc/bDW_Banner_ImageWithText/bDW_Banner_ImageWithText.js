...
export default class BDW_Banner_ImageWithText extends LightningElement{
    ...
    imageContentDesktop({ error, data }) {
        if (data) {
            ...
            console.log('Desktop Resource Test 1' + this.desktopbanner + 'Title' + this.title);
    ...
    {
        //this.gtmEvent();
        this.dniContactNumber = this.template.querySelector('.phone-link').innerHTML;
        this.gtmEvent();
    ...
    gtmEvent() {
        let contactUsCallEvent = new CustomEvent(
            'gtm__contactUsCall', {
            detail: {
                //contact: this.calltext1 + ' ' + contactno + ' ' + this.calltext2
                contact: this.calltext1 + ' ' + this.dniContactNumber + ' ' + this.calltext2
            }
        });
