...
import { registerListener, fireEvent, mergeFilters } from 'c/supubsub';
...
export default class SU_FilterSection extends LightningElement {
   ...
   @track dynamicClass = 'su__d-block su__text-nowrap '
    ...
    arr_StickyData = [];
    arr_StickyData1 = [];
    filtersPopupCheck = false;
    showchildsMerge = true;
    ...
    get defaultFilterCollapseCheckUp_Arrow() {
        return (this.defaultFilterCollapse ? 'arrow-position su__d-none su__cursor' : 'arrow-position su__d-block su__cursor su__loading-view');//{!if(v.defaultFilterCollapse == true,'su__d-none su__cursor','su__d-block su__cursor')}
    }
    ...
    get showFilterOrSticky(){
        this.showFilterOrStickyLabel = this.showFilter ? 'Filters' : 'Selected Filters';
        return this.showFilter || this.showAllStickyFacets;
    }

    connectedCallback() {
        // registerListener("setAggregationsData"+this.eventCode, this.setAggregationsData, this);
        registerListener("callclearfilter"+this.eventCode, this.clearFilterCalled, this);
        // registerListener("uncheckfromfiltersection"+this.eventCode, this.uncheckFilter, this);
        ...
        registerListener('bigscreen'+this.eventCode,this.setScreen,this);
    ...
    handleInsideFilters(event) {
        ...
        var data = this.template.querySelector(`div[data-id='${filterId}']`).classList.toggle("su__active-Filter-color");
    ...
    headerCheckoxChanged(event, flag, itemRemoved) {
        ...
        this.pageNum = 1;
        this.from = 0;
        //var setArray = this.aggregationsData || [];
        var currentClickedOrderIndex = this.aggregationsData.findIndex((f) => f.key == clickedFilter.parent);
        // this.currentClickedOrder = currentFilterOrder;
                    ...
                    if (clickedFilter.checked) {
                        ...
                        if (aggregations[index].sort && clickedFilter.Contentname == undefined) {
                            ...
                            if (clickedFilter.level == 1) {
                                ...
                                //var currentFilterOrder = this.currentClickedOrder;
        ...
        var currentFilterOrder = this.aggregationsData.find(function (f) { if (f.key == clickedFilter.parent) return f; })
        this.filterOrder = currentFilterOrder.order;
        //this.currentClickedOrder = currentFilterOrder;
        //helper.getValue(component, 'filterCheck', 'true', helper);need to pass to other component to get data
    ...
    traverseTheTree(childArray, clickedFilter, path, filtersInAggr, childrenArr) {
        for (var i = 0; i < childArray.length; i++) {
            //Object.assign(childArray[i], { 'selected': false});
            var filter = Object.assign({}, childArray[i]);
            if (childArray[i].Contentname == path[0]) {
                //childArray[i].selected = false;
                if (path.length > 1) {
    ...
    clearFilters() {
        fireEvent(this.pageRef, "clearFilterEvent"+this.eventCode, null);
    }
    ...
    collapseFilters(event) {
        ...
        var divTocollapse = this.template.querySelector(`[data-id="${childId}"]`);//document.getElementById(childId);
        var closeIcon = this.template.querySelector(`[data-id="${childId + '_toggleIconOn'}"]`);//document.getElementById(childId + '_toggleIconOn');
        var openIcon = this.template.querySelector(`[data-id="${childId + '_toggleIconOff'}"]`);//document.getElementById(childId + '_toggleIconOff');
    ...
    searchFilters(event){
        ...
        if(this.searchFacetIndexVal != '' && this.searchFacetIndexVal != searchFacetIndex){
            ...
            let searchFacetClass = this.aggregationsData[this.searchFacetIndexVal].facetFilterClass.split(' ')[0];
        ...
        let searchFacetClass = this.aggregationsData[searchFacetIndex].facetFilterClass.split(' ')[0];
        if (searchFacetClass) {
            let searchFacetInput = this.template.querySelector('div[data-id="searchFacetDiv-'+searchFacetClass+'"]');
    ...
    searchFilterSuggestions(key, searchStringFilter, event) {
        ...
        const found = this.mergedArrayReceived.filter(function(f) { return f.facetName === key && f.filterNewName.toLowerCase().indexOf(pagingAggregation.keyword.toLowerCase()) > -1});
                                    ...
                                    result.aggregationsArray.forEach(function (filter, index) {
                                        if (filter.key === key) {
                                            ...
                                            if(filter.values.length > 0){
                                                ...
                                                aggregationsData[index].values.some(function(value){
                                                ...
                                                aggregationsData[index].filterSuggestions = [];
                                                aggregationsData[index].filterSuggestions = filter.values;
                                                if(dropdownField.parentElement.classList) {
                                            ...
                                            aggregationsData[index].suggestionLength = aggregationsData[index].filterSuggestions && aggregationsData[index].filterSuggestions.length > 0
                                                ? true
                                                : false;
                                                if(aggregationsData[index].suggestionLength){
                                                    self.facetdrop ="su__search-facet-drop";
                                                    aggregationsData[index].filterSuggestClass='su__search-facet-input su__loading-view'
                                                }else{
                                                    self.facetdrop ="su__d-none"
                                                    aggregationsData[index].filterSuggestClass='su__search-facet-input-not su__loading-view'
