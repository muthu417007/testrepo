...
export default class Bwc_cartPage extends LightningElement {
    ...
    async handleCouponApplied(couponCodeReceived) {
        ...
        // Use a try-catch block to handle any errors during the execution
        try {
          await new Promise((resolve) => setTimeout(resolve, 2000));
      
          await this.getPriceAndQuantity();
          console.log("getPriceAndQuantity completed");
          this.template.querySelector('c-bwc_product-ship-type').filterCartItems(this._cartItems);
          console.log("filterCartItems completed");
          this.publishMessage();
          console.log("publishMessage completed");
        } catch (error) {
          console.error(error);
        }
      }
