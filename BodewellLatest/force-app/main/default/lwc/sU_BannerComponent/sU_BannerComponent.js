...
import supubsub from 'c/supubsub';
...
export default class SU_BannerComponent extends NavigationMixin(LightningElement) {
    ...
    searchString2;
    directionText = 'LTR';
    // showViewedResults = false;
    // showViewedResultsClass;
    CustSetting;
    ...
    timer = 0;
    dataObj; // object type declaration
    aggregation;// object type declaration
    //isEnableTitleRedirect=false;
    ...
    get recentSearchEndPoint() {
        return this.s3EndPoint + '/Assets/recent_search.svg';
    }
    get recentSearchAutosuggestion() {
        return this.s3EndPoint + '/Assets/auto_suggestion.svg';
    }
    get autosuggestListLength() {
        return this.autosuggestList.Length;
    }
    get showMetadataShowResultbody() {
        return (this.showMetadata || this.showResultbody) ? true : false;
    }
    ...
    connectedCallback() {
        ...
        // if (this.showViewedResults) {
        //     this.showViewedResultsClass = 'su__viewed-results slds-col slds-large-size--1-of-1 slds-small-size--1-of-1 slds-medium-size--1-of-1 su-searchBoxDiv su-nopad';
        // }
        // else {
        //     this.showViewedResultsClass = 'slds-col slds-large-size--1-of-1 slds-small-size--1-of-1 slds-medium-size--1-of-1 su-searchBoxDiv su-nopad';
        // }
    ...
    submitForm(event) {
        ...
        if(!this.autoSuggestionActive){
             fireEvent(this.pageRef, 'searchPage'+this.eventCode, { searchString: this.searchString2, isFreshSearch: -1});
        }
    }
    ...
    autoSuggestionClick(e) {
        ...
        if( e.value || e.currentTarget.dataset ){
        var href =  e.url || e.currentTarget.dataset.url ;
        if( e.value || e.currentTarget.dataset.value ){
        var id 
        if(e.currentTarget){
        ...
        var object =  e.currentTarget ? e.currentTarget.dataset.type : e.type;
        }
        ...
        // debugger;
        // var suggest = event && event.currentTarget ? event.currentTarget.dataset.suggest : null;
        // if (this.isEnableTitleRedirect && !suggest) {
        //     var target;
        //     if (event && event.currentTarget && event.currentTarget.closest('span') || (this.linkToOpen && this.dataObj)) {
        //         target = event && event.currentTarget ? event.currentTarget.closest('span') : null;
        //         var dataObj = this.dataObj || {};
        //         var data_Id = dataObj.data_Id || target.dataset.id || "";
        //         var data_type = dataObj.data_type || target.dataset.type || "";
        //         var data_index = dataObj.data_index || target.dataset.index || "";
        //         var data_rank = dataObj.data_rank || target.dataset.rank || "";
        //         var data_url = dataObj.data_url || target.dataset.url || "";
        //         var data_sub = dataObj.data_sub || target.dataset.subject || "";
        //         var result_count = dataObj.result_count || target.dataset.count || 0;
        //         var data_trackAnalytics = dataObj.data_trackAnalytics || target.dataset.trackanalytics || [];
        //         if (data_rank != '') {
        //             data_rank = (+data_rank) + 1;
        //         }
        //     }
        //     fireEvent(null, 'trackAnalytics'+this.eventCode, {type:'search', objToSend:{
    ...
    fillSearchBoxEventFire() {
        window.location.hash = encodeURIComponent("searchString=" + encodeURIComponent(this.searchString) + "&pageNum=" + this.getURLParameter("pageNum") + "&sortBy=" + this.getURLParameter("sortBy") + "&orderBy=desc&resultsPerPage=" + this.getURLParameter("resultsPerPage") + "&pageSizeAdv=" + this.getURLParameter("pageSizeAdv") + "&exactPhrase=" + encodeURIComponent(this.getURLParameter('exactPhrase')) + "&withOneOrMore=" + encodeURIComponent(this.getURLParameter('withOneOrMore')) + "&withoutTheWords=" + encodeURIComponent(this.getURLParameter('withoutTheWords')) + "&selectedType=" + encodeURIComponent(this.getURLParameter('selectedType')));
        fireEvent(this.pageRef, 'searchPage'+this.eventCode, { searchString: this.searchString }); this.mouseleft();
    }
    ...
    redirectFunc(valueToBeFilled) {
        var url;
        this.buttonPress = true;
        if (this.firstLoad == 1 || this.redirectThroughUrl) {
            if (window.location.hash == "") {
                window.location.hash = encodeURIComponent("searchString=" + encodeURIComponent(this.searchString));
            } else {
                window.location.hash = encodeURIComponent("searchString=" + encodeURIComponent(this.searchString) + "&pageNum=" + this.getURLParameter("pageNum") + "&sortBy=" + this.getURLParameter("sortBy") + "&orderBy=desc&resultsPerPage=" + this.getURLParameter("resultsPerPage") + "&pageSizeAdv=" + this.getURLParameter("pageSizeAdv") + "&exactPhrase=" + encodeURIComponent(this.getURLParameter('exactPhrase')) + "&withOneOrMore=" + encodeURIComponent(this.getURLParameter('withOneOrMore')) + "&withoutTheWords=" + encodeURIComponent(this.getURLParameter('withoutTheWords')) + "&selectedType=" + encodeURIComponent(this.getURLParameter('selectedType')));
            }
            url = this.currentCommunityURL + '' + window.location.hash + encodeURIComponent("&flag=0");
        }
        else
            window.location.hash = encodeURIComponent("searchString=" + encodeURIComponent(this.searchString) + "&pageNum=" + this.getURLParameter("pageNum") + "&sortBy=" + this.getURLParameter("sortBy") + "&orderBy=desc&resultsPerPage=" + this.getURLParameter("resultsPerPage") + "&pageSizeAdv=" + this.getURLParameter("pageSizeAdv") + "&exactPhrase=" + encodeURIComponent(this.getURLParameter('exactPhrase')) + "&withOneOrMore=" + encodeURIComponent(this.getURLParameter('withOneOrMore')) + "&withoutTheWords=" + encodeURIComponent(this.getURLParameter('withoutTheWords')) + "&selectedType=" + encodeURIComponent(this.getURLParameter('selectedType')));

        if (url == window.location.href) {
        } else if (this.firstLoad == 1 || this.redirectThroughUrl) {
            this.firstLoad = 0;
    ...
    enterKeySearch(event) {
        ...
        if (SearchQuery != null && SearchQuery != '' && SearchQuery != undefined && SearchQuery.length > 1) {
            if (event.keyCode == 13) {
                ...
                //     if(this.template.querySelector(".su-autoSuggest-element")){
                //     let length = this.template.querySelector(".su-autoSuggest-element").length;
                //     }
                //     // let length = this.template.querySelector(".su-autoSuggest-element").length; 
                //     let index = this.index;
                //     if (index > -1 && index < length) {
                //     if(this.template.querySelector(".su-autoSuggest-element") &&  this.template.querySelector(".su-autoSuggest-element")[index] && this.template.querySelector(".su-autoSuggest-element")[index].classList ){
                //         this.template.querySelector(".su-autoSuggest-element")[index].classList.remove('selected');
                //     }
                //     }
                //     if ((event.keyCode == 40 && index < length) || (event.keyCode == 38 && index > -1)) {
                //         event.keyCode == 40 ? index++ : index--;
                //         this.index = index;
                //         if(this.template.querySelector(".su-autoSuggest-element") && this.template.querySelector(".su-autoSuggest-element")[index] && this.template.querySelector(".su-autoSuggest-element")[index].classList ){
                //         this.template.querySelector(".su-autoSuggest-element")[index].classList.add('selected');
                //         }
                //         if(this.template.querySelector(".su-autoSuggest-element")){
                //             var autoSuggestElement = this.template.querySelector(".su-autoSuggest-element");
                //         }
                //         var newSearchQuery = autoSuggestElement[index].template.querySelector("span")[0].id 
                //         this.linkToOpen = autoSuggestElement[index].template.querySelector("span")[0].getAttribute('data-url');
                //         if (!this.isEnableTitleRedirect || !this.linkToOpen)
                //             this.searchString = newSearchQuery;
                //         var target = autoSuggestElement[index].template.querySelector("span")[0];
                //         var dataObj = {};
                //         if( this.template.querySelector(`[data-id="autosuggestElement"]`)&& this.template.querySelector(".su-autoSuggest-element.selected").offsetTop && this.template.querySelector(".su-autoSuggest-element.selected").offsetHeight){
                //             this.template.querySelector(`[data-id="autosuggestElement"]`).scrollTop = this.template.querySelector(".su-autoSuggest-element.selected").offsetTop + this.template.querySelector(".su-autoSuggest-element.selected").offsetHeight - this.template.querySelector(`[data-id="autosuggestElement"]`).offsetHeight;///document.getElementById("autosuggestElement").scrollTop = document.querySelector(".su-autoSuggest-element.selected").offsetTop + document.querySelector(".su-autoSuggest-element.selected").offsetHeight -document.getElementById("autosuggestElement").offsetHeight;
    ...
    autosearchFunc(searchType, runLoader) {
        this.autoSuggestionActive = false;
        this.sIndex = -1;
        let self = this;
        ...
        var data = JSON.stringify({
            ...
            "aggregations": this.getURLParameter('selectedType') ? JSON.parse(this.getURLParameter('selectedType')) : JSON.parse(aggrValue),
            "referrer": "",
            "exactPhrase": this.getURLParameter('exactPhrase'),
            "withOneOrMore": this.getURLParameter('withOneOrMore'),
            "withoutTheWords": this.getURLParameter('withoutTheWords'),
            "recommendResult": "",
            "indexEnabled": false,
            "sid": sid,
            "uid": this.uid,
            ...
            if (xmlHttp.readyState === 4) {
                if (xmlHttp.status === 200) {
                    ...
                    if (result.statusCode != 402) {
                        ...
                        //     self.showViewedResults = true;
                        // } else {
                        //     self.showViewedResults = false;
                        // }
                        ...
                        if (searchType === 'autosuggestion' && !self.buttonPress) {
                            ...
                            if(self.recommendedSearches) {
                                for (var i = 0; i < self.recommendedSearches.length; i++) {
                                    Object.assign(self.recommendedSearches[i], { 'recommendedRecentSearch': self.recommendedSearches[i].type == 'recentSearch' ? true : false });//if(self.recommendedItem.type == 'recentSearch', true,false)
                                    Object.assign(self.recommendedSearches[i], { 'recommendedAutoSuggestion': self.recommendedSearches[i].type == 'autoSuggestion' ? true : false });//{!if(recommendedItem.type == 'autoSuggestion', true,false)}
                                }
                            }
                                ...
                                for (var i = 0; i < self.autosuggestList.length; i++) {
                                    for (var j = 0; j < self.autosuggestList[i].metadata.length; j++) {
                                        if (self.autosuggestList[i].metadata[j].key == 'post_time') {
                                            Object.assign(self.autosuggestList[i].metadata[j], { 'metaKeyValBoolean': true });// {!if(metadata.key == 'post_time',true,false)}
                                            Object.assign(self.autosuggestList[i].metadata[j], { 'metaKeyVal': 'Created Date' });// {!if(metadata.key == 'post_time','Created Date',metadata.key)}
    
                                        }
                                        else {
                                            Object.assign(self.autosuggestList[i].metadata[j], { 'metaKeyValBoolean': false });// {!if(metadata.key == 'post_time',true,false)}
                                            Object.assign(self.autosuggestList[i].metadata[j], { 'metaKeyVal': 'Created Date' });// {!if(metadata.key == 'post_time','Created Date',metadata.key)}
    
                                        }
                                        // if (self.autosuggestList[i].metadata[j].value == '') {
                                        //     Object.assign(self.autosuggestList[i].metadata[j], { 'emptyMetaData': true });// {!if(metadata.key == 'post_time',true,false)}
                                        //     // Object.assign(self.autosuggestList[i].metadata[j], { 'emptyMetaData': 'Created Date' });// {!if(metadata.key == 'post_time','Created Date',metadata.key)}
    
                                        // }
                                        for (var k = 0; k < self.autosuggestList[i].metadata[j].value.length; k++) {
                                            if (k == 0) {
                                                Object.assign(self.autosuggestList[i].metadata[j].value[k], { 'singleValZero': true });// {!if(metadata.key == 'post_time',true,false)}
    
                                            }
                                            else {
                                                Object.assign(self.autosuggestList[i].metadata[j].value[k], { 'singleValZero': false });// {!if(metadata.key == 'post_time',true,false)}
